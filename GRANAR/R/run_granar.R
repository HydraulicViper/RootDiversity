...
run_granar <- function(params, tmp_sampl){
  ...
  params$value[params$type == "n_layers" & params$name == "cortex"] <- round(tmp_sampl$CF-1.5)
  params$value[params$type == "cell_diameter" & params$name == "stele"] <- tmp_sampl$X_size /8
  params$value[params$type == "layer_diameter" & params$name == "stele"] <- tmp_sampl$r_stele*2-1.75*tmp_sampl$OneC
  params$value[params$type == "cell_diameter" & params$name == "stele"] <- tmp_sampl$X_size /8
  params$value[params$type == "max_size" & params$name == "xylem"] <- tmp_sampl$X_size -1.5*params$value[params$type == "cell_diameter" & params$name == "stele"]
  ...
  if(tmp_sampl$radius > 0.7){
    tmp <- tibble(name = c("pith", "pith"), type = c("cell_diameter", "layer_diameter"), value = c(tmp_sampl$OneC, params$value[params$type == "layer_diameter" & params$name == "stele"]-3.5*params$value[params$type == "max_size" & params$name == "xylem"]))
